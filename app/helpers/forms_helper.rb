...
module FormsHelper
  ...
  def submit_button(text = 'Save', opts = {})
    ...
    tag.button(opts.merge(type: :submit, class: "btn btn-#{size} btn-primary", data: { disable_with: disable_with_text(opts.delete(:feedback_text)) })) do
      icon 'fas', icon, text
    end
  end

  def search_button(text = 'Search')
    tag.button(type: :submit, class: 'btn btn-default', data: { disable_with: disable_with_text('Search') }) do
